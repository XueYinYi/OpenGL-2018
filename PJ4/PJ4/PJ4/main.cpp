...
Display( )
{
    ...
    SetPointLight(GL_LIGHT1, 0., 1.3, 0., 0., 1., 1.);
    glLightfv(GL_LIGHT1, GL_POSITION, Array3(19.8 * cos((Time* -360) * 3.14159265359 )/ 180, 0., 19.8 * cos((Time* -360) * 3.14159265359 / 180)));
    
    
    
    
    SetSpotLight(GL_LIGHT2, 0., -3., 5., 0., 0., -3., 0., 1., 0.2);
    ...
    glPushMatrix();
    glTranslated(0,0,5. * cos((Time* -360) * 3.14159265359 / 180));
    glCallList(movingBox);
    glPopMatrix();
...
InitLists( )
{
    ...
    //drown the Dount
    Donut = glGenLists( 1 );
    glNewList(Donut, GL_COMPILE);
    glEnable( GL_LIGHTING );
    glShadeModel( GL_SMOOTH );
    glPushMatrix();
    SetMaterial(Red[0], Red[1], Red[2], 66);
    ...
    SetMaterial( Grey[0], Grey[1], Grey[2], 1);
    glTranslatef(6., 5, -13.);
    glutSolidTeapot(6);
    glDisable( GL_LIGHTING );
    glPopMatrix();
    ...
    movingBox = glGenLists( 1 );
    glNewList(movingBox, GL_COMPILE);
    glEnable( GL_LIGHTING );
    
    glPushMatrix();
    
    SetMaterial( Yellow[0], Yellow[1], Yellow[2], 64);
    glutSolidCube(2);
    glPopMatrix();
    glDisable( GL_LIGHTING );
    glEndList();
